...
def _get_row_subset_per_sheet(
    ...
    if isinstance(wb, openpyxl.workbook.Workbook):
        for row_iter in row_iterator:
            row_subset, date_columns_indices = __loop_and_fill_row_subsets(row_subset, enumerate(row_iter))
    else:
        row_subset, date_columns_indices = __loop_and_fill_row_subsets(row_subset, enumerate(row_iterator))
