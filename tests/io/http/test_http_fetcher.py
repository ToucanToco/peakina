...
def test_http_fetcher_with_query_string(http_path, mocker):
    http_path = http_path + "?foo=bar"
    fetcher = HttpFetcher()
    tmpfile = fetcher.open(http_path)
    assert pd.read_csv(tmpfile).shape == (800, 13)
    with pytest.raises(KeyError):
    ...
    assert tmpfile.name.endswith(".csv")
...
def test_http_fetcher_verify_no_ca_certs(http_path, path, mocker):
    ...
    fetcher = HttpFetcher(verify=True, retries=False)
